...
%global _privatelibs libprotobuf.*|libmysqlharness\.so.*|libmysqlharness_stdx\.so.*|libmysqlharness_tls\.so.*|libmysqlrouter\.so\..*|libmysqlrouter_.*\.so\..*
...
Provides:       community-mysql-libs = %{version}-%{release}
Provides:       community-mysql-libs%{?_isa} = %{version}-%{release}
...
# Ignore absolute RPATH followed by $ORIGIN in mysqld.
...
* Mon May 11 2020 Murthy Sidagam <venkata.sidagam@oracle.com> - 8.0.21-1
...
- Added back support for cluster
...
- added a separate macro "mysqld_group" to be able to define the
  user group of the mysql user separately, if desired.
